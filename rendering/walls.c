...
float calculate_distance(t_ray_data ray)
{
    if (ray.side == 0)
        return (ray.side_dist.x - ray.delta_dist.x) * GRID_DIST;
    else
        return (ray.side_dist.y - ray.delta_dist.y) * GRID_DIST;
}
...
void draw_col(t_ray_data	ray, int col)
{
	...
	wall_height = (GRID_DIST / calculate_distance(ray)) * ((WIN_WIDTH / 2) / tan(FOV / 2));

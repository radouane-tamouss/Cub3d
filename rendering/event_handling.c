...
void update_movement() {
  // the parallax effecy achieved by moving the gun in the opposite direction of
  // the player the gun_offset_x is used to move the gun in the opposite
  // direction of the player the gun_offset_x is then multiplied by 0.9 to slow
  // down the movement of the gun
...
int handle_keys(int keycode, void *garbage) {
  ...
  //   {
  //     if ((get_data()->front_ray.object_hitted == 1 ||
  //     get_data()->front_ray.object_hitted == 2) && get_data()->front_ray.dist
  //     < 2 * GRID_DIST)
  //     {
